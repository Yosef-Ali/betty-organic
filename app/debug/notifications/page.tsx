...
// Fully client-side mock notification debugger
function MockNotificationDebugger() {
  const [pendingOrders, setPendingOrders] = useState<any[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [logs, setLogs] = useState<string[]>([]);
  const [connectionStatus, setConnectionStatus] =
    useState<string>('SUBSCRIBED');
...
// Fully client-side mock notification tester
        ...
        <div>
          ...
          <div className="bg-muted p-2 rounded-md">
            {pendingOrders.length === 0 ? (
              ...
              <div className="space-y-2">
                {pendingOrders.map(order => (
                  <div key={order.id} className="bg-card p-2 rounded-md border">
                    <div className="font-medium">{order.display_id}</div>
                    <div className="text-sm text-muted-foreground">
                      {new Date(order.created_at).toLocaleString()}
                    </div>
