        ...
        () => [
            ...
            },
            {
                ...
                cell: ({ row }: { row: any }) => {
                    ...
                    // Function to handle sending invoice via WhatsApp with PDF
                    const handleSendInvoiceWhatsApp = async () => {
                        try {
                            const order = row.original;

                            // Always use admin/default phone number for manual WhatsApp sharing
                            const adminPhone = '251944113998'; // Default admin phone number
                            ...
                            // Get customer contact info for the message
                            const customerPhone = order.customer?.phone || order.guest_phone || 'Not provided';
                            const customerEmail = order.customer?.email || order.guest_email || 'Not provided';
                            ...
                            console.log('Generating PDF and preparing WhatsApp message for admin:', {
                            ...
                            // First, generate and download the PDF invoice
                            const pdfData = {
                                orderId: order.display_id || order.id,
                            ...
                            let pdfFilename = `Betty_Organic_Invoice_${pdfData.orderId}.pdf`;
                            ...
                            if (response.ok) {
                                ...
                                if (result.success && result.pdfBase64) {
                                    ...
                                    console.warn('PDF generation failed, continuing with text message');
                                }
                            } else {
                                console.warn('PDF API failed, continuing with text message');
                            }

                            // Create WhatsApp message for admin with PDF attachment instructions
                            const whatsappText = `
🌿 *Betty's Organic Store* 🌿
📋 *Order Invoice with PDF*

📅 *Date:* ${pdfData.orderDate}
⏰ *Time:* ${pdfData.orderTime}
🔢 *Order ID:* ${pdfData.orderId}
👤 *Customer:* ${customerName}
📞 *Customer Phone:* ${customerPhone}
✉️ *Customer Email:* ${customerEmail}

� *PDF Invoice:* ${pdfFilename}
💡 *Note:* PDF invoice has been downloaded to your device. Please attach it to this chat.
...
✨ Ready for delivery! Please contact customer and attach the PDF invoice. ✨

_PDF downloaded automatically - attach it to this WhatsApp chat_
                            `.trim();

                            try {
                                // Open WhatsApp with admin phone number and enhanced message
                    ...
                    const handleDownloadInvoice = async () => {
                        try {
                            console.log('Desktop Download Invoice clicked for order:', row.original.id);
                            const order = row.original;
                            ...
                            console.log('Calling PDF API with data:', invoiceData);
                            const response = await fetch('/api/generate-invoice-pdf', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                            ...
                            if (response.ok) {
                                const result = await response.json();
                                console.log('PDF API result:', { success: result.success, hasBase64: !!result.pdfBase64, filename: result.filename });
                    ...
                    return (
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                                ...
                                <DropdownMenuItem onClick={handleOpenInvoiceModal}>
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end" className="w-48">
                                                ...
                                                <DropdownMenuItem
                                                    onClick={async (e) => {
                                                        e.stopPropagation();
                                                        console.log('Mobile Download Invoice clicked for order:', order.id);
                                                        ...
                                                        try {
                                                            console.log('Mobile PDF API call with data:', invoiceData);
                                                            const response = await fetch('/api/generate-invoice-pdf', {
                                                                method: 'POST',
                                                                headers: { 'Content-Type': 'application/json' },
                                                                body: JSON.stringify(invoiceData)
                                                            ...
                                                            console.log('Mobile PDF API response status:', response.status);
                                                            if (response.ok) {
                                                                const result = await response.json();
                                                                console.log('Mobile PDF API result:', { success: result.success, hasBase64: !!result.pdfBase64, filename: result.filename });
                                                                if (result.success && result.pdfBase64) {
                                                                    // Create download link for PDF
                                                                    const link = document.createElement('a');
                                                                    link.href = `data:${result.contentType};base64,${result.pdfBase64}`;
                                                                    ...
                                                                    toast.success("Invoice downloaded successfully!");
                                                                    console.log('Mobile PDF download triggered successfully');
                                                                } else {
                                                                    console.error('Mobile PDF generation failed:', result);
                                                                    toast.error("Failed to generate PDF invoice");
                                                                }
                                                            } else {
                                                                const errorText = await response.text();
                                                                console.error('Mobile PDF API failed:', response.status, errorText);
                                                                toast.error("Failed to generate invoice");
                                                            }
                                                        } catch (error) {
                                                            console.error('Mobile Error generating invoice:', error);
                                                            toast.error("Error generating invoice");
                                                        }
                                                    }}
                                                >
